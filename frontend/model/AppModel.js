...
export default class AppModel {
    ...
    static async getDests() {            // changed
        try{
            const destResponse = await fetch('http://localhost:8080/dests'); // get запрос по-умолчанию
    ...
    static async getFerrys() {            // changed
        try{
            const ferryResponse = await fetch('http://localhost:8080/ferrys'); // get запрос по-умолчанию
    ...
    static async getLimits({tripID} = { tripID: null}) {
        try{
            const limitResponse = await fetch(`http://localhost:8080/limit/${tripID}`); // get запрос по-умолчанию
    ...
    static async getTrips() {            // changed
        try{
            const tripResponse = await fetch('http://localhost:8080/trips'); // get запрос по-умолчанию
    ...
    static async updateLoad({loadID, name, type, car_type, position = -1} = {   //пока что только название и позиция, добавить остальные штуки
        ...
        try{
            const updateLoadResponse = await fetch(
                `http://localhost:8080/loads/${loadID}`,
                {
                    method: 'PATCH',
