...
export default class App {
    ...
    onEscapeKeydown = (event) => {
        if (event.key === 'Escape') {
            ...
            document.querySelector('.trip-adder__btn').style.display = 'inherit';
        }
    };
    ...
    onInputKeydown = async (event) => {  //создаем новый пункт
        ...
        if (event.target.value) {
            ...
            try {
                const addDestResult = await AppModel.addDests({
                    destID, name: event.target.value,
                });
        ...
        document.querySelector('#dest-adder').style.display = 'inherit';
    };
            ...
            if ((tripLimits['car_place'] < newCars || tripLimits['load_place'] < newLoad) && (srcTripID !== destTripID)) {
                ...
                if (srcTripID !== destTripID) {
                    ...
                    await AppModel.moveLoad({
                        loadID: movedLoadID, srcTripID, destTripID
                    });
    ...
    editLoad = async ({loadID, newLoadName, newLoadType, newLoadCarType}) => {
        ...
        try {
            const updateLoadResult = await AppModel.updateLoad({
                loadID, name: newLoadName, type: newLoadType, car_type: newLoadCarType
            });
            ...
            if (destID && ferryID) {
                ...
                try {
                    const addTripResult = await AppModel.addTrips({
                        tripID, destID, ferryID, position: this.#trips.length
                    });
    ...
    initAddLoadModal() {
        ...
        const okHandler = () => {
            ...
            if (tripID && modalInputName.value && modalInputType.value) {
                this.#trips.find(trip => trip.tripID === tripID).appendNewLoad({
    ...
    initEditLoadModal() {
        ...
        const okHandler = () => {
            ...
            if (loadID && modalInputName.value && modalInputType.value) {
                this.editLoad({
                    loadID,
                    newLoadName: modalInputName.value,
    ...
    async init() {
        ...
        document.addEventListener('dragover', (evt) => {
            ...
            if (curDroppable === draggedElementPrevList) {
                ...
                const nextElement = (currentElement === draggedElement.nextElementSibling) ? currentElement.nextElementSibling : currentElement;
